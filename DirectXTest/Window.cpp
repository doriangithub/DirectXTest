...
Window::Window(int width, int height, const char* name)
	...
	height( height )
{
	// calculate windows size based on desired client region size
	RECT wr;
	wr.left = 100;
	...
	// throw std::runtime_error("Something happaned");
	// throw 45626;
...
LRESULT WINAPI Window::HandleMsgSetup(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	...
	if (msg == WM_NCCREATE)
	{
		// extract prt to window class from creation data
		const CREATESTRUCTW* const pCreate = reinterpret_cast<CREATESTRUCTW*>(lParam);
		Window* const pWnd = static_cast<Window*>(pCreate->lpCreateParams);
		// set WinAPI - maneged user data to store ptr to window class
...
LRESULT Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	...
	/********** KEYBOARD MESSAGES ***********/
	case WM_KEYDOWN:
	// syskey command need to be handled to track ALT key (VK_MENU)
	case WM_SYSKEYDOWN:
	...
	case WM_MOUSEMOVE:
	{
		...
		// in client region -> log move, and log enter + capture move ( if not prev...
...
const char* Window::Exception::what() const noexcept
{
	...
	oss << GetType() << std::endl
		...
		<< "[Descroption] " << GetErrorString() << std::endl
		<< GetOriginString();
	whatBuffer = oss.str();
...
int Window::ProcessMessage()
{
	...
	while (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
	{
		...
		// TranslateMessage will post auxilliary WM_CHAR message from key msg
